            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-8">
                <div className="hidden lg:flex gap-6 text-sm text-gray-300">
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-400"></div><span>1,247 Online</span></div>
                  <div>🎮 89 Games</div>
                  <div>🏟️ 23 Arenas</div>
                </div>
                <div className="hidden md:flex items-center gap-6 bg-white bg-opacity-10 rounded-lg px-4 py-2 border border-white border-opacity-20">
                  <div className="text-center"><div className="text-sm font-bold text-green-400">{calculateWinRate()}%</div><div className="text-xs text-gray-400">Win Rate</div></div>
                  <div className="text-center"><div className="text-sm font-bold text-blue-400">{playerData.wins}/{playerData.losses}/{playerData.draws}</div><div className="text-xs text-gray-400">W/L/D</div></div>
                  <div className="text-center"><div className="text-sm font-bold text-purple-400">Lv.{playerData.level}</div><div className="text-xs text-gray-400">Level</div></div>
                  {playerStats && (<>
                    <div className="text-center"><div className="text-sm font-bold text-yellow-400">{playerStats.bestScore > 999999 ? `${(playerStats.bestScore / 1000000).toFixed(1)}M` : `${Math.floor(playerStats.bestScore / 1000)}K`}</div><div className="text-xs text-gray-400">Best Score</div></div>
                    <div className="text-center"><div className="text-sm font-bold text-cyan-400">{formatPlayTime(playerStats.totalPlayTime)}</div><div className="text-xs text-gray-400">Playtime</div></div>
                  </>)}
                </div>
              </div>
              <div className="flex items-center gap-4 bg-white bg-opacity-10 rounded-xl px-4 py-2 border border-white border-opacity-20">
                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 flex items-center justify-center text-white font-bold">👤</div>
                <div>
                  <div className="text-white font-semibold">{playerData.username}</div>
                  <div className="text-sm" style={{ color: getRankColor(playerData.rank) }}>{playerData.rank} • {playerData.elo} ELO</div>
                </div>
              </div>
            </div>
          </div>
