...
// Arena page specific interfaces
...
const OnlinePortal: React.FC<OnlinePortalProps> = ({ onBack, onStartGame }) => {
  ...
  // Helper function to generate mock arena players
  const generateMockArenaPlayers = (count: number, hostName: string): ArenaPlayer[] => {
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      return {
        ...
        joinedAt: new Date(Date.now() - Math.random() * 600000), // Joined within last 10 minutes
        isHost
      };
    });
  };
  ...
  // Handler for viewing arena details
  const handleViewArena = (arena: Arena) => {
    // Convert Arena to DetailedArena with enhanced mock data
    ...
    // 20% chance player is already in arena
    setIsPlayerInSelectedArena(Math.random() > 0.8); 
    setCurrentSection('arena');
  };

  // Generate contextual arena descriptions
  ...
  // Handler for joining arena
  const handleJoinArena = () => {
    if (selectedArena && playerData) {
      // Add current player to arena
  ...
  // Handler for leaving arena
  const handleLeaveArena = () => {
    if (selectedArena && playerData) {
      // Remove current player from arena
      setSelectedArena({
        ...selectedArena,
